...
member1_walking_data = pd.concat([
    pd.read_csv('UserData/MPWalkingBackPocket.csv', nrows=M1SamplingFactor*dataRange),
    pd.read_csv('UserData/MPWalkingFrontPocket.csv', nrows=M1SamplingFactor*dataRange),
    pd.read_csv('UserData/MPWalkingJacketPocket.csv', nrows=dataRange),
    pd.read_csv('UserData/MPWalkingHand.csv', nrows=dataRange)
], ignore_index=True)

# Jumping Data
member1_jumping_data = pd.concat([
    pd.read_csv('UserData/MPJumpingBackPocket.csv', nrows=M1SamplingFactor*dataRange),
    pd.read_csv('UserData/MPJumpingFrontPocket.csv', nrows=M1SamplingFactor*dataRange),
    pd.read_csv('UserData/MPJumpingJacketPocket.csv', nrows=M1SamplingFactor*dataRange),
    pd.read_csv('UserData/MPJumpingHand.csv', nrows=M1SamplingFactor*dataRange)
], ignore_index=True)
...
member2_walking_data = pd.concat([
    pd.read_csv('UserData/AMWalkingBackPocket.csv', nrows=M2SamplingFactor*dataRange),
    pd.read_csv('UserData/AMWalkingFrontPocket.csv', nrows=M2SamplingFactor*dataRange),
    pd.read_csv('UserData/AMWalkingJacketPocket.csv', nrows=M2SamplingFactor*dataRange),
    pd.read_csv('UserData/AMWalkingHand.csv', nrows=M2SamplingFactor*dataRange)
], ignore_index=True)

member2_jumping_data = pd.concat([
    pd.read_csv('UserData/AMJumpingBackPocket.csv', nrows=M2SamplingFactor*dataRange),
    pd.read_csv('UserData/AMJumpingFrontPocket.csv', nrows=M2SamplingFactor*dataRange),
    pd.read_csv('UserData/AMJumpingJacketPocket.csv', nrows=M2SamplingFactor*dataRange),
    pd.read_csv('UserData/AMJumpingHand.csv', nrows=M2SamplingFactor*dataRange)
], ignore_index=True)
...
member3_walking_data = pd.concat([
    pd.read_csv('UserData/XZWalkingBackPocket.csv', nrows=M3SamplingFactor*dataRange),
    pd.read_csv('UserData/XZWalkingFrontPocket.csv', nrows=M3SamplingFactor*dataRange),
    pd.read_csv('UserData/XZWalkingJacketPocket.csv', nrows=M3SamplingFactor*dataRange),
    pd.read_csv('UserData/XZWalkingHand.csv', nrows=M3SamplingFactor*dataRange)
], ignore_index=True)


member3_jumping_data = pd.concat([
    pd.read_csv('UserData/XZJumpingBackPocket.csv', nrows=M3SamplingFactor*dataRange),
    pd.read_csv('UserData/XZJumpingFrontPocket.csv', nrows=M3SamplingFactor*dataRange),
    pd.read_csv('UserData/XZJumpingJacketPocket.csv', nrows=M3SamplingFactor*dataRange),
    pd.read_csv('UserData/XZJumpingHand.csv', nrows=M3SamplingFactor*dataRange)
], ignore_index=True)
...
with h5py.File('./accelerometer_data.h5', 'w') as hdf:
    ...
    # Create a subgroup for the dataset
    ...
    for member_name, member_data in all_data.items():
        for activity in ['walking', 'jumping']:
            ...
            # Define window overlap sizes
            window_size = 500  # 5 seconds
            overlap_size = 100  # 1 second

            # Segment the data
            num_segments = (len(data) - window_size) // overlap_size + 1
            segments = [data[(i * overlap_size):(i * overlap_size + window_size)] for i in range(num_segments)]
    ...
    # Create subgroups for train and test datasets
    train_group = dataset_group.create_group('train')
    test_group = dataset_group.create_group('test')

    # Add walking and jumping datasets to train and test subgroups
